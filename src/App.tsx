...
export default function App() {
  ...
  const toggleDarkMode = () => {
    setIsDarkMode((prev) => {
      const next = !prev;
      if (typeof window !== 'undefined') {
        localStorage.setItem('fm_theme', next ? 'dark' : 'light');
        if (next) {
          document.documentElement.classList.add('dark');
          const meta = document.getElementById('theme-color-meta');
          if (meta) meta.setAttribute('content', '#0b1320');
        } else {
          document.documentElement.classList.remove('dark');
          const meta = document.getElementById('theme-color-meta');
          if (meta) meta.setAttribute('content', '#1769ff');
        }
      }
      return next;
    });
      ...
      <div className="bg-gradient-to-r from-[#10233f] via-[#123970] to-[#10233f] text-white py-2 px-4 text-xs font-semibold border-b border-slate-800">
        <div className="max-w-[1180px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-2 text-center sm:text-left">
          ...
          <div className="flex items-center gap-3 shrink-0">
            {/* Quick Dark Mode Toggle in Top Ribbon */}
            <button
              onClick={toggleDarkMode}
              className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-slate-800/80 hover:bg-slate-700 text-slate-200 transition text-[11px] font-semibold border border-slate-700 active:scale-95"
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#e6edf7] shadow-sm">
        <div className="max-w-[1180px] mx-auto px-5 py-3.5 flex items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-2.5">
            ...
            {/* Dark Theme Option at Top */}
            <button
              onClick={toggleDarkMode}
              className="inline-flex items-center gap-1.5 bg-[#f0f5ff] hover:bg-[#e4efff] text-[#10233f] border border-[#d6e5fb] px-3 py-2 rounded-xl font-bold text-xs transition active:scale-95 shadow-sm"
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white border-t border-[#e6edf7] px-5 py-3 shadow-xl flex flex-col animate-in fade-in slide-in-from-top-2 duration-150">
            ...
            <div className="py-2.5 my-1 border-t border-[#e6edf7] flex items-center justify-between">
              <span className="text-xs font-bold text-[#61708a]">Display Theme</span>
              <button
                onClick={toggleDarkMode}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#d6e5fb] text-xs font-bold text-[#10233f] bg-[#f0f5ff] transition active:scale-95"
              >
                {isDarkMode ? (
                  <>
      ...
      <main className="flex-1">
        ...
        {activeTab === 'home' && (
          <div className="space-y-16 sm:space-y-20">
            ...
            <section className="max-w-[1180px] mx-auto px-5 pt-8 sm:pt-12">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
                ...
                <div className="lg:col-span-6 space-y-6">
                  ...
                  <p className="text-base sm:text-lg leading-relaxed text-[#61708a] max-w-[540px]">
                    Practical technology education for school students (Grades 1–10) in Ananth Nagar Phase 2 & Electronic City, Bengaluru. Kids don’t just stare at screens — they assemble real circuits, build autonomous rovers, program sensors, train AI models, and engineer real-world capstone projects.
                  </p>
            ...
            {/* Quick FAQ Section on Home for Fast Crawling & Rich Snippets */}
            <section id="faq" className="max-w-[1180px] mx-auto px-5">
              <div className="bg-white border border-[#e6edf7] rounded-[28px] p-6 sm:p-9 shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#e6edf7]">
                  <div>
                    <div className="inline-flex items-center gap-1.5 bg-[#eaf2ff] text-[#1458d7] rounded-full px-3 py-1 font-bold text-xs mb-2">
                  ...
                  {faqs.slice(0, 4).map((f, idx) => (
                    <div key={idx} className="py-4">
                      ...
                      >
                        ...
                        <span className="text-lg font-bold text-[#1769ff] shrink-0">
                          {openFaq === idx ? '−' : '＋'}
                        </span>
                      </button>
                      {openFaq === idx && (
                        <p className="pt-2 text-xs sm:text-sm text-[#61708a] leading-relaxed animate-in fade-in duration-150">
                          {f.a}
                        </p>
                      )}
                    </div>
